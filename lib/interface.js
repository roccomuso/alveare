...
class CliInterface {
  ...
  welcome () {
    this.stdout.write(welcomeMessage.yellow)
    this.rl.prompt()
  }
    ...
    if (command[0] === '.') {
      switch (command.slice(1)) {
        case 'help':
          ...
          break
        case 'credit':
          this.write(`Rocco Musolino (@roccomuso) - github.com/roccomuso/alveare`.grey)
          break
        case 'quit':
        case 'q':
          process.exit(0)
      ...
      if (command !== '') {
        this.write(`"${command}" is not a valid command dude, sorry`.yellow)
      }
    }
    this.rl.prompt()
  }
